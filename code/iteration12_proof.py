#!/usr/bin/env python3
"""Iteration 12: First Canonical Production Research Proof — Fixed Execution Script.

Fixes from previous attempt:
1. sys.path includes /root/prop-desk/futures_lab BEFORE any futures_lab import
2. _synthetic_spec_for_file imported and used to create FuturesSpec per ticker
3. run_backtest called with correct 4-positional-arg signature: (df, spec, strategy, params)
4. run_backtest returns (metrics, trades, equity_series) tuple — handled correctly
"""
...
# ======================================================================
# CRITICAL: sys.path setup BEFORE any futures_lab import
# This was the bug in the previous attempt — futures_lab wasn't on sys.path.
# ======================================================================
FUTURES_LAB_ROOT = "/root/prop-desk/futures_lab"
STRATEGY_COMBINE_PROJECT = "/root/prop-desk/strategy_combine"
STRATEGY_COMBINE_CODE = STRATEGY_COMBINE_PROJECT + "/code"
sys.path.insert(0, FUTURES_LAB_ROOT)
sys.path.insert(0, STRATEGY_COMBINE_CODE)
sys.path.insert(0, STRATEGY_COMBINE_PROJECT)

# Now safe to import pandas
import pandas as pd

# ======================================================================
# Import core modules from strategy_combine
# ======================================================================
...
# ======================================================================
# Import backtest engine from futures_lab (now on sys.path)
# ======================================================================
from futures_lab import ZOO_PARAM_GRIDS, run_backtest, _synthetic_spec_for_file
...
TIMEFRAMES = ["15m"]  # bounded: only 15m for this proof
HORIZONS = [60]       # bounded: only 60d for this proof
...
MAX_PARAMS = 2        # 2 param combos per strategy to fit bounded budget
MIN_BARS = 100
MIN_TRADES = 5
MIN_PF = 1.0
INITIAL_CASH = 1_000_000
MAX_CANDIDATES = 80   # bounded proof: 50-100 candidates
...
def phase3_execute(plan_configs: List[Dict[str, Any]], datasets: Dict[tuple, Path]) -> ResearchRun:
    ...
    try:
        ...
        # --- Execute Backtests ---
        non_skipped = [cfg for cfg in plan_configs if cfg.get("config_key", "") not in skipped_keys]
        log(f"Executing backtests for {len(non_skipped)} non-skipped candidates...")

        # Pre-create FuturesSpec per ticker using _synthetic_spec_for_file
        ticker_specs = {ticker: _synthetic_spec_for_file(ticker) for ticker in UNIVERSE}
        log(f"Created synthetic specs for {list(ticker_specs.keys())}")
        ...
        for cfg in plan_configs:
            ...
            spec = ticker_specs.get(ticker)
            if spec is None:
                log(f"  SKIP {config_key}: no spec for {ticker}")
                continue

            try:
                t0 = time.time()
                # FIX: Correct 4-positional-arg call: (df, spec, strategy, params)
                # Returns (metrics, trades, equity_series) tuple
                metrics, trades, equity_series = run_backtest(
                    df, spec, strat, params, initial_cash=INITIAL_CASH
                )
                ...
                log(f"  ERROR {config_key} ({ticker}/{strat}): {e}")
                failed += 1
                candidate = {
                    "config_key": config_key,
                    "ticker": ticker,
...
def main():
    ...
    log("ITERATION 12: FIRST CANONICAL PRODUCTION RESEARCH PROOF (FIXED)")
    ...
    try:
        ...
        log(f"WARNING: Lifecycle build failed: {e}")
        results["lifecycle_build"] = {"error": str(e)}
    ...
    try:
        ...
        results["post_health"] = {"error": str(e)}

    # Phase 8: Reconciliation
    phase8_reconcile()

    # Save results
    proof_dir = REPORT_DIR / "proofs" / "iteration12"
    proof_dir.mkdir(parents=True, exist_ok=True)
    results_path = proof_dir / "proof_result.json"
    # Also save to run dir
    run_results_path = rr.run_dir / "proof_results.json"
    ...
    run_results_path.write_text(results_json, encoding="utf-8")
    log(f"\nResults saved to {results_path}")
    log(f"Results also saved to {run_results_path}")
...
if __name__ == "__main__":
    results = main()
    print("\n=== PROOF SUMMARY ===")
    print(json.dumps({k: v for k, v in results.items() if k not in ("all_candidates", "data_manifest")}, indent=2, default=str))
