...
for line in text.splitlines():
    ...
    if collect:
        ...
        if len(parts) >= 16 and parts[0] in CORE:
            rows.append({'ticker': parts[0], 'tf': parts[1], 'strategy': parts[2]})
...
for i, r in enumerate(rows):
    ...
    ax.plot(x, cum, linewidth=2.6, color=color,
            label=f"{ticker} {strategy} {tf} | trades={len(trades)} | PnL={float(metrics.get('total_pnl') or 0):+.0f}₽ | PF={float(metrics.get('profit_factor') or 0):.2f} | WR={float(metrics.get('win_rate_pct') or 0):.0f}%")
    ax.fill_between(x, 0, cum, color=color, alpha=0.08)

ax.axhline(0, color='#888', linewidth=.9)
ax.set_title('GAZP / SBER / LKOH top strategies — trade-by-trade cumulative PnL', fontsize=16, fontweight='bold')
ax.set_xlabel('closed trade #')
ax.set_ylabel('cumulative PnL, ₽')
ax.grid(alpha=.25)
ax.legend(loc='best', fontsize=7)
REPORT.mkdir(parents=True, exist_ok=True)
out = REPORT / 'gazp_sber_lkoh_top_overlay.png'
