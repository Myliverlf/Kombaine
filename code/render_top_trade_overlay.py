...
# Keep top 5 to avoid clutter
rows = rows[:5]

fig, ax = plt.subplots(figsize=(15, 8), dpi=180)
plt.style.use('dark_background')
colors = ['#00e5ff', '#76ff03', '#ffca28', '#ff4081', '#b388ff']
...
for i, r in enumerate(rows):
    ...
    ax.plot(x, cum, linewidth=2.6, color=color,
            label=f"{ticker} {strategy} {tf} | PnL={float(metrics.get('total_pnl') or 0):+.1f} ₽ | PF={float(metrics.get('profit_factor') or 0):.2f} | trades={len(trades)}")
...
ax.set_title('Top strategies overlay — cumulative PnL by closed trades', fontsize=16, fontweight='bold')
ax.set_xlabel('closed trade #')
ax.set_ylabel('cumulative PnL, ₽')
ax.grid(alpha=.25)
ax.legend(loc='best', fontsize=8)
REPORT.mkdir(parents=True, exist_ok=True)
out = REPORT / 'top_trade_overlay.png'
