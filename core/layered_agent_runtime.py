...
class LayeredAgentRuntime:
    ...
    def _build_followup_from_ledger(self, ledger_context: list[dict[str, Any]], fallback_goal: str) -> list[dict[str, Any]]:
        tasks: list[dict[str, Any]] = []
        for item in reversed(ledger_context[-10:]):
            evidence = item.get("l3", {}).get("evidence", {}) if isinstance(item.get("l3"), dict) else {}
            claim = str(evidence.get("claim", "")).strip()
            summary = str(item.get("l3", {}).get("result", {}).get("result_summary", "")).strip() if isinstance(item.get("l3"), dict) else ""
            goal = claim or summary
            if not goal:
                continue
            tasks.append({
                "goal": goal[:180],
                "why": "continue from prior evidence",
                "role": "L2",
                "inputs": [summary or claim],
                "expected_output": ["next evidence", "short summary"],
            })
        if not tasks and fallback_goal:
            tasks.append({
                "goal": fallback_goal,
                "why": "restart from current goal",
                "role": "L2",
                "inputs": [fallback_goal],
                "expected_output": ["evidence", "next action"],
            })
        return tasks[:5]
    ...
    def run_iterative(self, max_cycles: int = 3) -> Dict[str, Any]:
        ...
        for cycle in range(max_cycles):
            ...
            agent_context = {
                ...
                "paper_only": True,
                "forbidden_actions": ["live order", "broker mutation"],
            }
            ...
            # FIX(handoff): layers run sequentially and each layer receives the
            # previous layer's real output. Previously all three got identical
            # context, so results were never actually transferred between levels.
            l1_raw = l1_proc.run(build_role_prompt("L1", agent_context))
            l1_res = normalize_external_payload(l1_raw.payload, "L1")
            if not l1_raw.ok:
                l1_res["convergence"] = {"status": "stuck", "reason": f"process_exit_{l1_raw.returncode}"}
                l1_res["blockers"].append(f"exit_code_{l1_raw.returncode}")

            l2_context = dict(agent_context)
            l2_context["upstream"] = {"role": "L1", "summary": l1_res.get("summary"), "result": l1_res.get("result"), "next_tasks": l1_res.get("next_tasks")}
            l2_raw = l2_proc.run(build_role_prompt("L2", l2_context))
            l2_res = normalize_external_payload(l2_raw.payload, "L2")
            if not l2_raw.ok:
                l2_res["convergence"] = {"status": "stuck", "reason": f"process_exit_{l2_raw.returncode}"}
                l2_res["blockers"].append(f"exit_code_{l2_raw.returncode}")

            l3_context = dict(agent_context)
            l3_context["upstream"] = {"role": "L2", "summary": l2_res.get("summary"), "result": l2_res.get("result"), "next_tasks": l2_res.get("next_tasks"), "l1_summary": l1_res.get("summary")}
            l3_raw = l3_proc.run(build_role_prompt("L3", l3_context))
            l3_res = normalize_external_payload(l3_raw.payload, "L3")
            if not l3_raw.ok:
                l3_res["convergence"] = {"status": "stuck", "reason": f"process_exit_{l3_raw.returncode}"}
                l3_res["blockers"].append(f"exit_code_{l3_raw.returncode}")

            self._attach_external_results(l1, l2, l3, l1_res, l2_res, l3_res, l1_raw, l2_raw, l3_raw)
            followup_tasks = self._collect_followup_tasks(l1_res, l2_res, l3_res)
            if not followup_tasks:
                followup_tasks = self._build_followup_from_ledger(ledger_context, self.router.state.goal)
            if followup_tasks:
    ...
    def _attach_external_results(self, l1, l2, l3, l1_res, l2_res, l3_res, l1_raw=None, l2_raw=None, l3_raw=None):
        # FIX(always-PASS): status must reflect real process exit and whether
        # the agent actually produced a summary. Fabricated fallbacks are FAIL.
        def _status(res, raw):
            if raw is not None and not raw.ok:
                return "FAIL"
            if res.get("summary_fabricated"):
                return "FAIL"
            return "PASS"

        self.orch.attach_result(l1, build_result(
            task_id=f"task::{l1}",
            status=_status(l1_res, l1_raw),
