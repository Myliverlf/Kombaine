...
class TestDetect:
    ...
    def test_stuck_respects_threshold(self):
        ...
        # 2 decisions, not stuck (below threshold=3), no results → UNKNOWN
        # FIX(false-converged): decisions without results are NOT convergence.
        assert report.status == Status.UNKNOWN
    ...
    def test_converged_no_results(self):
        ...
        # FIX(false-converged): zero results must not read as converged.
        assert report.status == Status.UNKNOWN
        assert "no-evidence" in report.reason.lower()
